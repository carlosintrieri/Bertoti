...
const WeatherTime = () => {
  ...
  const [horaCidade, setHoraCidade] = useState(null);
  const [eDia, setEDia] = useState(true);
  const [eDiaCidade, setEDiaCidade] = useState(null);
  const [cidades, setCidades] = useState(['São Paulo', 'Rio de Janeiro', 'Brasília', 'Salvador', 'Curitiba']);

  // Estado para estabilizar o relógio e evitar piscadas
  const [horaCidadeEstabilizada, setHoraCidadeEstabilizada] = useState(null);
  const [periodoEstabilizado, setPeriodoEstabilizado] = useState(null);

  // Relógio local e tema visual
  useEffect(() => {
    const timer = setInterval(() => {
      const now = new Date();
      setHoraAtual(now);
      
      // O tema visual é baseado no horário local
  ...
  // Relógio adaptado ao UTC da cidade
  useEffect(() => {
    if (dadosClima && dadosClima.atual && dadosClima.atual.timezone_offset !== undefined) {
      // Primeiro cálculo para estabilizar o relógio quando a cidade muda
      const now = new Date();
      const utcTime = now.getTime() + (now.getTimezoneOffset() * 60000);
      const cidadeTime = new Date(utcTime + (dadosClima.atual.timezone_offset * 1000));
      
      const hora = cidadeTime.getHours();
      const ehDia = hora >= 6 && hora < 18;
      
      // Definir valores estabilizados que não vão mudar até nova busca
      setHoraCidadeEstabilizada(cidadeTime);
      setPeriodoEstabilizado(ehDia);
      
      // Atualização contínua do relógio
      const updateCidadeTime = () => {
        const now = new Date();
        const utcTime = now.getTime() + (now.getTimezoneOffset() * 60000);
        const cidadeTime = new Date(utcTime + (dadosClima.atual.timezone_offset * 1000));
        
        setHoraCidade(cidadeTime);
        
        const hora = cidadeTime.getHours();
        setEDiaCidade(hora >= 6 && hora < 18);
      };
      
      updateCidadeTime();
      const timer = setInterval(updateCidadeTime, 1000);
  ...
  const gerarDadosSimulados = (nomeCidade) => {
    ...
    return {
      atual: {
        ...
        lat: (Math.random() * 10 - 5 + -23.5505).toFixed(4),
        lon: (Math.random() * 10 - 5 + -46.6333).toFixed(4),
  ...
  const getIconeAnimado = (condicao, eDia, tamanho = 24) => {
    condicao = condicao?.toLowerCase() || '';
  ...
  return (
      ...
      style={{
        background: dadosClima 
          ? (periodoEstabilizado 
              ...
              : 'linear-gradient(135deg, #1a237e, #000051)') 
          : (eDia 
              ? 'linear-gradient(135deg, #bbdefb, #e3f2fd)'  
              : 'linear-gradient(135deg, #1a237e, #000051)') 
      }}
    >
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        {(dadosClima && periodoEstabilizado) || (!dadosClima && eDia) ? (
          Array(15).fill().map((_, i) => (
            <div 
              key={i}
          ...
          <div className="flex items-center p-3 rounded-xl shadow-lg" style={{
            ...
            <div>
              <div className="text-2xl font-mono" style={{ color: textColor }}>
                {horaCidadeEstabilizada 
                  ? horaCidade.toLocaleTimeString('pt-BR') 
                  : horaAtual.toLocaleTimeString('pt-BR')}
              </div>
              <div style={{ color: textColor }} className="text-sm opacity-80">
                {horaCidadeEstabilizada 
                  ? (periodoEstabilizado ? 'Período diurno' : 'Período noturno') 
                  : (eDia ? 'Período diurno' : 'Período noturno')}
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="flex items-center p-3 rounded-xl shadow-lg" style={{
                ...
                <div>
                  <div style={{ color: textColor }} className="text-2xl font-mono">
                    {horaCidade ? horaCidade.toLocaleTimeString('pt-BR') : horaAtual.toLocaleTimeString('pt-BR')}
                  </div>
                  <div style={{ color: textColor }} className="text-sm opacity-80">
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  <div className="text-center md:text-left flex-1">
                    ...
                    <div className="mt-2 inline-block px-3 py-1 rounded-full" style={{
                      ...
                      {periodoEstabilizado ? 'Dia' : 'Noite'}
                    </div>
                  </div>
                  
                  <div className="flex justify-center items-center w-40 h-40">
                    {getIconeAnimado(dadosClima.atual.descricao, periodoEstabilizado)}
                  </div>
